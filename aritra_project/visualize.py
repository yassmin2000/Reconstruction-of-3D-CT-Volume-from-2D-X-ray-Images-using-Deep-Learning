...
def my_vis(epoch_values,
    ...
    plt.plot(epoch_values, val_loss_values)
    ...
    # =========================
    plt.figure()
    plt.title('Model PSNR (dB)')
    plt.plot(epoch_values, metric_values)
    plt.plot(epoch_values, val_metric_values)
    plt.legend(['Train', 'Val'])
    plt.xlabel('Epoch')
    plt.ylabel('PSNR (dB)')
    plt.savefig(os.path.join(results_path, "PSNR.png"))
    plt.close()
    ...
    plt.plot(epoch_values, metric1_values)
    plt.plot(epoch_values, val_metric1_values)
    plt.legend(['Train', 'Val'])
    plt.xlabel('Epoch')
    plt.ylabel('SSIM')
    plt.savefig(os.path.join(results_path, "SSIM.png"))
    plt.close()
    ...
    torch.save(model.state_dict(),
               os.path.join(results_path, "output_latest.pth"))

    if best_metric_coeff == 1:
        torch.save(model.state_dict(),
                   os.path.join(results_path, "output_best.pth"))
